...
parser.add_argument('-r', '--radius', help='dot radius to generate mask labels, specify only if masks need to be generated', type=int)
...
#%%
# Generate .gif masks for each training image, containing a white dot at each of the label coordinates
if not args.radius:
  ...
  for img_id in train_labels.index:
    ...
    if len(coords) == 8:
      ...
      draw.ellipse((coords[0]-r, coords[1]-r, coords[0]+r, coords[1]+r), fill=(255))
      draw.ellipse((coords[2]-r, coords[3]-r, coords[2]+r, coords[3]+r), fill=(255))
      draw.ellipse((coords[4]-r, coords[5]-r, coords[4]+r, coords[5]+r), fill=(255))
      draw.ellipse((coords[6]-r, coords[7]-r, coords[6]+r, coords[7]+r), fill=(255))
